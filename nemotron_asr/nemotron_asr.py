...
# # Cache-Aware RNNT Pipeline on Modal
#
# This example shows how to run the NeMo cache-aware RNNT pipeline on Modal
# using NVIDIA's Parakeet streaming model.
...
#
# We define a dataclass that mirrors the structure of cache_aware_rnnt.yaml
# This makes it easier to work with in Python code
...
# ## Pipeline Runner
#
# We define the main Pipeline class as a Modal Cls.
# This class loads the pipeline and runs inference on an audio file.
...
class NemotronASR:
    ...
    async def load(self):
        ...
        print(f"  State pool size: {len(self.pipeline._state_pool)}")
        print(f"  Active streams: {len(self.pipeline._streaming_request_generator.streams)}")
        print(f"  ✅ Ready for production streams!")
        
        # Set timing reference point (after warmup)
        self.start_time = time.perf_counter()
        print(f"\n⏱️  Start time set. All timing will be relative to this point.\n")
        ...
        print("\nMulti-client infrastructure initialized!")
        ...
        async def run_with_websocket(ws: WebSocket):
            ...
            async def recv_loop(ws, stream_id, transcription_queue):
                ...
                Supports both msgpack-encoded auBATCHdio (new) and raw PCM bytes (old) for backwards compatibility
                
                MICRO-BATCHING: Accumulates multiple chunks before appending to pipeline
                to reduce per-call overhead. Batch size = att_context_size[1] + 1
                """
                # Micro-batching setup
                MICRO_BATCH_SIZE = self.cfg.streaming.att_context_size[1] + 1
                audio_buffer = b""  # Accumulate chunks here as bytes
                num_buffer_samples = 0
                
                # # TIMING INSTRUMENTATION (COMMENTED OUT - using inference loop timing now)
                # chunk_count = 0
                # timing_stats = {
                #     'ws_receive': [],
                #     'msgpack_decode': [],
                #     'numpy_convert': [],
                #     'pipeline_append': [],
                #     'buffer_concat': [],  # Time to concatenate buffered chunks
                # }
                # REPORT_INTERVAL = 100  # Log stats every N chunks
                
                elapsed = time.perf_counter() - self.start_time
                print(f"[+{elapsed:7.3f}s] Stream {stream_id}: Micro-batching enabled (batch_size={MICRO_BATCH_SIZE})")
                ...
                try:
                    while True:
                        ...
                        # OPTIMIZED MESSAGE PROCESSING - Single-pass with minimal overhead
                        # Strategy:
                        # 1. Decode all msgpack messages once (unavoidable per-message cost)
                        # 2. Check for Marker (rare, early exit)
                        # 3. Process Audio messages with minimal branching
                        # 4. Use numpy efficiently (zero-copy views, vectorized ops)
                        ...
                        # if chunk_count > 0 and chunk_count % REPORT_INTERVAL == 0:
                        #     elapsed = time.perf_counter() - self.start_time
                        #     report = []
                        ...
                        #             report.append(f"{name}: avg={avg:.3f}ms min={min_t:.3f}ms max={max_t:.3f}ms p95={p95:.3f}ms")
                        #     
                        #     print(f"[+{elapsed:7.3f}s] ⏱️  Stream {stream_id} recv_loop @ {chunk_count} chunks (batch_size={MICRO_BATCH_SIZE}, buffer={len(audio_buffer)}):")
                        #     for line in report:
                        #         print(f"         {line}")
                        #     
                    ...
                    # # Print final timing summary for this stream
                    # elapsed = time.perf_counter() - self.start_time
                    # print(f"[+{elapsed:7.3f}s] ⏱️  Stream {stream_id} recv_loop FINAL SUMMARY ({chunk_count} total chunks, batch_size={MICRO_BATCH_SIZE}):")
                    # 
                    # # Aggregate all remaining stats
                    # for name, times in timing_stats.items():
                    #     if times:
                    #         avg = sum(times) / len(times)
                    #         min_t = min(times)
                    #         max_t = max(times)
                    #         total = sum(times)
                    #         print(f"         {name}: avg={avg:.3f}ms min={min_t:.3f}ms max={max_t:.3f}ms total={total:.1f}ms")
            ...
            async def send_loop(ws, transcription_queue, stream_id):
                ...
                OPTIMIZATIONS:
                - Uses PRE-ENCODED messages from route_outputs (batch encoding)
                - Sends in client's preferred format (msgpack or JSON)
                - No redundant encoding in this loop
                - NEVER drops messages - every transcription result is sent
                """
                step_num = 0
            ...
            tasks = []
            try:
                # Only 2 loops now: recv and send
                # Inference happens in centralized loop
                tasks = [
                ...
                # recv_loop will signal send_loop to stop via None in queue
    ...
    async def route_outputs(self, outputs_with_timestamps):
        ...
        OPTIMIZATIONS:
        - Minimizes lock hold time with quick snapshot
        - BATCH ENCODING: Pre-encodes messages in BOTH formats (msgpack + JSON)
          Each client receives messages in their preferred format
        ...
        async def send_to_queue(stream_id, output_dicts, queue):
            ...
            IMPORTANT: No timeout - we NEVER drop transcription results.
            If the queue fills up, we apply backpressure by blocking here.
            """
    ...
    async def centralized_inference_loop(self):
        """
        GPU-paced inference loop.
        Processes frames as fast as GPU allows - no artificial delays.
        This prevents backlog accumulation and maintains real-time performance.
        """
        ...
        while self.inference_running:
            try:
                ...
                last_client_count = active_clients
                ...
                try:
                    ...
                    if self.exhaustive_batching:
                        ...
                        # This ensures no stream gets skipped and all stay synchronized
                        first_batch = True
                        while True:
                            try:
                                # [TIMING] Process batch with GPU operations
                                # Note: GPU work (transfer + inference) is fast with pinned memory and non-blocking transfers
                                ...
                                # This ensures WebSocket handshakes and I/O can proceed
                                await asyncio.sleep(0)
                                ...
                                # Attach timestamps to outputs AFTER incrementing
                        ...
                        try:
                            ...
                            if outputs:
                                ...
                                # Attach timestamps to outputs BEFORE incrementing
                                for output in outputs:
                                    current_timestamp = cycle_timestamps.get(output.stream_id, 0.0)
                    ...
                    if all_outputs_with_timestamps:
                        ...
                        # Show cycle summary (commented out - using timing summary now)
                        # print(f"[+{elapsed:7.3f}s] Cycle {batch_count+1}: {num_batches} batches, {total_outputs} outputs, {unique_streams} streams")
                        # 
                        # # Show each batch in the cycle
                        # for i, batch_info in enumerate(batches_in_cycle):
                        #     print(f"    Batch {i+1}/{num_batches}: {batch_info['size']} outputs → streams {batch_info['streams']}")
                        
                        # [TIMING] Route all outputs (non-blocking and parallel)
    ...
    def transcribe_file(self, audio_url: str) -> dict:
        ...
        # Calculate RTFX
        rtfx = data_dur / inference_time if inference_time > 0 else float('inf')
...
# ## Local entrypoint
#
# For testing, we can run this locally
