...
web_image = (
    ...
    .uv_pip_install("pipecat-ai[daily,nvidia,riva,openai,silero]", "fastapi")
)
...
app = modal.App("nvidia-daily-modal", secrets=[modal.Secret.from_name("daily")])
...
@app.function(image=bot_image, min_containers=1, buffer_containers=1, single_use_containers=True, region = 'us-west')
async def bot_runner(room_url, token):
    ...
    try:
        ...
        print(f"Starting bot process: -u {room_url} -t {token}")
...
async def start(data: ConnectData):
    ...
    launch_bot_func = modal.Function.from_name("nvidia-daily-modal", "bot_runner")
...
@app.function(image=web_image, min_containers=1, region='us-west', buffer_containers=1, single_use_containers=True)
@modal.asgi_app()
def fastapi_app():
