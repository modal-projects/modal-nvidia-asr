...
BYTES_PER_SAMPLE = 2
FRAME_LEN_SEC = 0.080
PARAKEET_RT_STREAMING_CHUNK_SIZE = int(FRAME_LEN_SEC * SAMPLE_RATE) *  BYTES_PER_SAMPLE * NUM_REQUIRED_BUFFER_FRAMES 
...
with image.imports():
    ...
    from .asr_utils import preprocess_audio, int2float
...
class Transcriber:
    ...
    async def load(self):
        ...
        self.diar_model = SortformerEncLabelModel.from_pretrained("nvidia/diar_streaming_sortformer_4spk-v2.1").eval().to(torch.device("cuda"))
        self.asr_model = ASRModel.from_pretrained("nvidia/multitalker-parakeet-streaming-0.6b-v1").eval().to(torch.device("cuda"))
        
        self.cfg = OmegaConf.structured(MultitalkerTranscriptionConfig())
        self.diar_model = MultitalkerTranscriptionConfig.init_diar_model(self.cfg, self.diar_model)
        self.multispk_asr_streamer = SpeakerTaggedASR(self.cfg, self.asr_model, self.diar_model)
        ...
        for audio_data in chunk_audio(audio_bytes, PARAKEET_RT_STREAMING_CHUNK_SIZE):
            transcript, stream_id = await self.transcribe(audio_data, step_num, stream_id)
            step_num += 1
            stream_id = 0
        ...
        async def run_with_websocket(ws: WebSocket):
            ...
            async def recv_loop(ws, audio_queue):
                ...
                while True:
                    ...
                    # streaming_buffer.add_audio(data)
